    ...
    <div className="flex absolute h-full w-full top-0 left-0 bg-darkgray-100">
      ...
      <div className={`flex-1 p-6 ${bgClr} text-[#fbfbfb]`}>
        ...
        <div className="flex items-center gap-[0.8vw] border-b border-gray-500 w-full p-[1vw]">
          ...
          {/* Save Button */}
          {isChanged && <div className="bg-blue-500 pl-[1.5vw] pr-[1.5vw] pt-[0.3vw] pb-[0.3vw] select-none cursor-pointer font-bold rounded-sm ml-auto">Save</div>}
        </div>
        ...
        <div className="space-y-[0.2vw]">
          <div className={`flex items-center gap-[0.5vw] px-[0.5vw] py-[0.3vw] border ${borderClr} rounded-sm w-1/3`}>
            <img src={profileImage} className="w-[2.5vw] h-[2.5vw] rounded-full" />
            <p className="font-bold">{pending[0]}</p>
            <div className={`ml-auto ${fgClr} font-bold px-[1vw] py-[0.7vw] cursor-pointer rounded-sm hover:bg-red-400 transition`}>Remove</div>
          </div>
        </div>

        {/* Pending Requests */}
        <h2 className="text-xl font-bold mt-[5vh] mb-[1vh]">Pending Requests</h2>
      ...
      {/* Right Panel */}
      <div className={`w-1/5 p-[1vw] ${bgClr} border-l-[0.1vw] ${borderClr} text-white`}>
        ...
        {/* Toggle Buttons */}
        <div className={`flex gap-[0.5vw] mb-[3vh] select-none rounded-sm p-[0.2vw] ${fgClr}`}>
          <div onClick={() => setSelectedRole("students")} 
            className={`${selectedRole === "students" ? `bg-blue-400` : "bg-transparent"} flex-1 text-center py-[0.5vw] rounded-sm cursor-pointer font-bold`}>
            Students</div>
          <div onClick={() => setSelectedRole("teachers")} className={`${selectedRole === "teachers" ? `bg-blue-400` : "bg-transparent"} flex-1 text-center py-[0.5vw] rounded-sm cursor-pointer font-bold`}>
            Teachers</div>
        </div>
        ...
        <div className="space-y-[0.2vw]">
          {selectedRole == "students" && filteredAccounts.length > 0 ? (
            filteredAccounts.map((account, index) => (
              <div key={index} className={`flex items-center gap-[0.5vw] p-[0.5vw] border ${borderClr} rounded-sm select-none hover:${fgClr} transition`}>
                <img src={profileImage} className="w-[2vw] h-[2vw] rounded-full" alt="Profile" />
                <p className="font-bold">{account}</p>
                <div className={`ml-auto ${fgClr} px-[1vw] py-[0.7vw] rounded-sm border ${borderBgClr} cursor-pointer ${!pending.includes(account) && "hover:bg-blue-400 transition"}`}
                  onClick={()=>{if(!pending.includes(account)) {setPending((prevItems) => [...prevItems, account]);}}}>
