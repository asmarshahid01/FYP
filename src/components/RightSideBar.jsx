			...
			async (searchQuery, searchType) => {
				...
				try {
					...
					const response = await axios.get(
						`http://localhost:4000/api/search/fypSearch`,
						{
			...
			<div className='space-y-[0.2vw]'>
				{selectedRole == 'students' && results.length > 0 ? (
					results.map((account, index) => (
						...
						>
							<img
								src={
									account?.imageUrl
										? `http://localhost:4000${account.imageUrl}`
										: profileImage
								}
					...
					<p className='text-gray-500'>
						{searchQuery.length > 0 && selectedRole != 'teachers'
							? 'No accounts found'
							: ''}
					</p>
				)}
				{selectedRole == 'teachers' && results.length > 0 ? (
					...
					<p className='text-gray-500'>
						{searchQuery.length > 0 && selectedRole != 'students'
							? 'No accounts found'
							: ''}
					</p>
				)}
